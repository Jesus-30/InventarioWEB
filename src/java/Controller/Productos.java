...
public class Productos extends HttpServlet {
    ...
    protected void listarProductos(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException{
        ...
        session.setAttribute("listas", producto.Listar());
        RequestDispatcher dispatcher = getServletContext().getRequestDispatcher("/Vistas-Productos/listarProducto.jsp");
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (parametro.equals("crear")) {
            ...
            String pagina = "/Vistas-Productos/crearProducto.jsp?id_prod="+id+"&&nombre_prod="+nombre+"&&stock_prod="+stock+"&&precio_prod="+precio+"&&unidad_med="+unidad+"&&estado_prod="+estado+"&&categoria_prod="+categoria+"&&desc_prod="+descripcion+"&&RadioV=1";
            RequestDispatcher dispatcher = getServletContext().getRequestDispatcher(pagina);
            dispatcher.forward(request, response);
            ...
            ProductoDAO producto = new ProductoDAOImplementar();
            producto.borrarPro(eliminar_id);
            this.listarProductos(request, response);
        }
    }
